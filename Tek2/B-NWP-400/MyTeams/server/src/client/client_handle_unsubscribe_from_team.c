...
** client_handle_unsubscribe_from_team.c
** File description:
** client_handle_unsubscribe_from_team.c
*/
...
)
{
    ...
    sv_unsubscribe_from_team_packet_t res;
    ...
    res.type = PACKET_TYPE_SV_UNSUBSCRIBE_FROM_TEAM;
    res.team_subscription = subscription;
    stream_send_packet(self->stream, (cl_sv_packet_t *)&res);
    state_remove_subscription(self->state, self->user, team);
    server_event_user_unsubscribed(STRING_CDATA(team->uuid),
