...
)
{
    ...
    if (channel == nullptr)
        return;
    res.type = PACKET_TYPE_SV_LIST_THREADS;
    res.threads = state_get_threads_of_channel(self->state, channel);
    stream_send_packet(self->stream, (cl_sv_packet_t *)&res);
    vec_destroy(res.threads);
}
