...
static bool send_file(tcp_stream_t *stream, char *path)
{
    ...
    while (bytes > 0) {
        if (!full_write(*stream, buffer, bytes)) {
            close(fd);
            return false;
        }
        bytes = read(fd, buffer, 1024);
...
static pid_t fork_and_send(client_t *client, char *path)
{
    ...
    if (pid != 0)
        return pid;
    if (!send_file(&data->stream, full_path))
        exit(1);
    exit(0);
}
...
static bool on_handling_command(client_t *client, char *path)
{
    ...
    if (true_path == NULL || strncmp(true_path, root, strlen(root)) != 0)
        return control_send(ctrl, "550 Failed to send file.\r\n");
    if (access(true_path, R_OK) != 0)
        return control_send(ctrl, "550 Failed to access file.\r\n");
...
bool command_retr(client_t *client, char **args)
{
